...
def main() -> None:
    ...
    sysusers = (ROOT / "packaging/sysusers/elliot.conf").read_text(encoding="utf-8")
    ...
    results.append(
        scenario(
            ...
            f"ExecStart={CURRENT_LINK}/.venv/bin/siper-daemon" in unit
            and f"ExecStartPre={CURRENT_LINK}/.venv/bin/siper-preflight --service" in unit,
        )
    )
    results.append(
        scenario(
    ...
    results.append(
        scenario(
            ...
            "g elliot " in sysusers
            and "g elliot-admin " in sysusers
            and "usermod -a -G elliot" in installer
            and "usermod -a -G elliot-admin" in installer,
        )
    )
    results.append(
        scenario(
            "gui_launcher_targets_installed_console_script",
            f"ENTRY={CURRENT_LINK}/.venv/bin/siper-gui" in launcher
    ...
    results.append(
        scenario(
            ...
            str(INSTALL_ROOT) == "/opt/elliot"
            and str(SERVICE_UNIT_PATH) == "/etc/systemd/system/elliot.service"
            and str(DESKTOP_ENTRY_PATH) == "/usr/share/applications/siper.desktop"
            and IPC_GROUP == "elliot",
        )
    )

    with tempfile.TemporaryDirectory(prefix="elliot-stage13-layout-") as temporary:
