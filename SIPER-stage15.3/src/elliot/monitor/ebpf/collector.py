"""Filtered BCC collector for ELLIOT runtime telemetry.
...
class EBPFCollector:
    ...
    def load(self) -> bool:
        ...
        try:
            ...
            if "ELLIOT_RENAME_PROBES" in source and not self._rename_tracepoints:
                ...
                logger.warning("eBPF degraded: %s", self.degraded_reason)
                return False
            source = source.replace("ELLIOT_RENAME_PROBES", self._build_rename_probe_source(self._rename_tracepoints))
    ...
    def _detect_exec_filename_field(cls) -> str | None:
        ...
        """
        override = os.environ.get("ELLIOT_EXEC_FILENAME_FIELD")
    ...
    def _detect_rename_tracepoints(cls) -> list[str]:
        ...
        """
        override = os.environ.get("ELLIOT_RENAME_TRACEPOINTS")
        allowed = ("rename", "renameat", "renameat2")
    ...
    def start(self) -> bool:
        ...
        self._poll_thread = threading.Thread(target=self._poll_loop, daemon=True, name="elliot-ebpf-poll")
