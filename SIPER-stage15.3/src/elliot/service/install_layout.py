"""Canonical production-installation paths and validation helpers for Siper (ELLIOT project)."""
...
INSTALL_ROOT = Path("/opt/elliot")
RELEASES_DIRECTORY = INSTALL_ROOT / "releases"
CURRENT_LINK = INSTALL_ROOT / "current"
SERVICE_UNIT_PATH = Path("/etc/systemd/system/elliot.service")
GUI_LAUNCHER_PATH = Path("/usr/local/bin/siper-gui")
DESKTOP_ENTRY_PATH = Path("/usr/share/applications/siper.desktop")
SYSUSERS_PATH = Path("/usr/lib/sysusers.d/elliot.conf")
RUNTIME_DIRECTORY = Path("/run/elliot")
STATE_DIRECTORY = Path("/var/lib/elliot")
LOG_DIRECTORY = Path("/var/log/elliot")
IPC_SOCKET = RUNTIME_DIRECTORY / "elliot.sock"
IPC_GROUP = "elliot"
ADMIN_GROUP = "elliot-admin"
...
def installation_manifest(
    *,
    ...
    return {
        ...
        "daemon_entry_point": str(CURRENT_LINK / ".venv/bin/siper-daemon"),
        "gui_entry_point": str(CURRENT_LINK / ".venv/bin/siper-gui"),
