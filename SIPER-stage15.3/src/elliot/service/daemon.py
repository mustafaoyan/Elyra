"""Privileged Siper daemon entry point.
...
logger = logging.getLogger("elliot.service.daemon")


class ElliotDaemonState:
...
class ElliotDaemon:
    ...
    def _spawn(self, target, name: str) -> threading.Thread:
        thread = threading.Thread(target=target, name=f"elliot-{name}", daemon=True)
        thread.start()
        return thread
    ...
    def stop(self) -> None:
        logger.info("Stopping Siper daemon")
