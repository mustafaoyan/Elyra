...
from elliot.monitor.ebpf.collector import EBPFCollector, EbpfFilterConfig
...
def test_missing_bcc_is_safe_degraded(tmp_path: Path):
    ...
    import elliot.monitor.ebpf.collector as module
    original = module.BPF
    module.BPF = None
    try:
...
def test_exec_tracepoint_field_detection_uses_pardus_member(monkeypatch):
    monkeypatch.delenv("ELLIOT_EXEC_FILENAME_FIELD", raising=False)
...
def test_exec_tracepoint_field_override(monkeypatch):
    monkeypatch.setenv("ELLIOT_EXEC_FILENAME_FIELD", "__data_loc_filename")
...
def test_rename_tracepoint_detection_and_source_generation(monkeypatch):
    monkeypatch.setenv("ELLIOT_RENAME_TRACEPOINTS", "rename,renameat,renameat2")
...
def test_rename_detection_does_not_guess_when_tracefs_is_unreadable(monkeypatch):
    monkeypatch.delenv("ELLIOT_RENAME_TRACEPOINTS", raising=False)
    monkeypatch.setattr(
        EBPFCollector,
        "_tracepoint_exists",
