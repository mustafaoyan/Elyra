...
ELLIOT uses one local transport: `/run/elliot/elliot.sock` with newline-delimited UTF-8
...
- socket: `0660`;
- sensitive operations: root or `elliot-admin`;
...
manual scan size limit is 512 MiB. Root and `elliot-admin` may scan other regular files.
