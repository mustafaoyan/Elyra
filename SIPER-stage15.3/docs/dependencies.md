...
|---|---|---|
| `numpy` | Byte histograms and Shannon entropy calculations | `elliot.analyzer.static_analyzer` |
| `customtkinter` | Desktop GUI widgets | `elliot.gui.view` |
| `matplotlib` | Block-entropy graph rendering | `elliot.gui.view` |
| `python-magic` | MIME detection through libmagic | `elliot.analyzer.static_analyzer` |
| `pyelftools` | ELF header, section and segment parsing | `elliot.analyzer.static_analyzer`, `elliot.analyzer.pre_execution` |
...
The BCC Python module is intentionally **not** installed from PyPI. ELLIOT expects the
