...
|---|---|---:|---|---|
| `list_runtime_states` was accepted by the protocol and routed by the daemon but absent from the authorization action table | `src/elliot/service/authorization.py` | High | Installed integration checks and GUI runtime-state reads were rejected as an unknown action | Classify it as a read-only action and add cross-layer contract tests |
