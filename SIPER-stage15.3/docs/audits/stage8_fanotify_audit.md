...
Scope: `src/elliot/monitor/fanotify/`, its static-analysis adapter, and the daemon ownership boundary. No eBPF, response-engine, systemd, or production installation claim is made here.
...
| Incomplete feature | ELLIOT excluded only its exact daemon PID | `controller.py:50-52` | Medium | Helper descendants could recursively trigger monitoring | Add bounded `/proc` ancestry exclusion and installation/data/log path exclusion |
