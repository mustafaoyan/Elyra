"""Read-only verification of an installed ELYRA systemd deployment."""
...
def _systemctl_state(property_name: str) -> tuple[bool, str]:
    completed = _run(["systemctl", property_name, "elyra.service"])
...
def collect_installation_report(*, expected_user: str | None = None) -> dict[str, Any]:
    ...
    if release is not None:
        ...
        if manifest_check.safe:
            try:
                ...
                manifest_ok = (
                    manifest_payload.get("project") == "ELYRA"
    ...
    daemon_path = CURRENT_LINK / ".venv/bin/elyra-daemon"
    gui_path = CURRENT_LINK / ".venv/bin/elyra-gui"
    preflight_path = CURRENT_LINK / ".venv/bin/elyra-preflight"
    for name, path in (
    ...
    show = _run(["systemctl", "show", "elyra.service", "--property=ExecStart", "--value"])
    ...
    journal = _run(["journalctl", "-u", "elyra.service", "-n", "20", "--no-pager"])
    checks.append(
        _record(
            "journalctl_access",
