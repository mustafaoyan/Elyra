"""Official unprivileged client for ELYRA's Unix-domain-socket API."""
...
DEFAULT_SOCKET_PATH = "/run/elyra/elyra.sock"
...
class IpcClient:
    ...
    def request(self, action: str, params: Mapping[str, Any] | None = None) -> Any:
        ...
        try:
            ...
            raise IpcClientError("ELYRA daemon socket was not found") from exc
        except PermissionError as exc:
            raise IpcClientError("Permission denied while connecting to ELYRA IPC") from exc
        except (ConnectionRefusedError, ConnectionResetError, socket.timeout, OSError) as exc:
            raise IpcClientError(f"ELYRA IPC connection failed: {exc}") from exc
