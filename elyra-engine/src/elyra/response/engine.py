"""Authorised Stage 11 response execution for ELYRA.
...
class ResponsePolicy:
    ...
    def load(cls, path: str | os.PathLike[str] | None = None) -> "ResponsePolicy":
        if path is None:
            text = files("elyra.config").joinpath("response.default.json").read_text(
                encoding="utf-8"
            )
        else:
...
class PidfdProcessController:
    ...
    def terminate(self, expected: ProcessIdentity, policy: ResponsePolicy) -> dict[str, Any]:
        ...
        if any(
            ...
            raise ResponseSafetyError("refusing to terminate an ELYRA protected executable")
    ...
    def _execute_quarantine(self, state: Mapping[str, Any], recommendation: str) -> ActionResult:
        try:
            ...
            if self._protected_path(binding.file.path):
                raise ResponseSafetyError("ELYRA protected paths cannot be quarantined")
