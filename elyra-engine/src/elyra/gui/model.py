"""Unprivileged GUI model using only ELYRA's official Unix-socket API."""
...
logger = logging.getLogger("elyra.gui.model")
...
class PardusModel:
    ...
    def _send_request(self, action: str, params: dict[str, Any] | None = None) -> dict[str, Any]:
        try:
            ...
            logger.error("Elyra IPC error during %s: %s", action, exc)
            return {
                "ok": False,
