"""Modern, local-only desktop dashboard for the ELYRA defence engine.
...
logger = logging.getLogger("elyra.gui.view")
_BaseView = ctk.CTk if ctk is not None else object


class ElyraView(_BaseView):
    ...
    def __init__(self, *, banner_text: str | None = None) -> None:
        ...
        self.title("ELYRA — Local Zero-Day Defence")
    ...
    def _choose_scan(self, callback: Callable[[str], None]) -> None:
        selected = filedialog.askopenfilename(title="Select a file for ELYRA analysis")
        if selected:
            callback(selected)
