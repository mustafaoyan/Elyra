...
`elyra-daemon` runs as root because fanotify, BCC/eBPF and protected response
operations require privileges. `elyra-gui` runs as the desktop user. The only
supported transport is `/run/elyra/elyra.sock`, owned by root and the
`elyra` group with mode `0660`. Linux `SO_PEERCRED` identifies the connecting
PID, UID and GID.
