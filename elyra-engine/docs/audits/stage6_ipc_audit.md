# ELYRA Stage 6 Daemon and IPC Audit
...
| Socket mode was `0770` | `service/ipc_server.py:37` | Medium | Execute bits are meaningless for sockets and obscure intended access | Changed to `0660` with dedicated `elyra` group |
...
| Sensitive operations relied only on action names, without a versioned request contract | `authorization.py:10-64` | High | Restore, delete, and enforcement changes lacked consistent validation | Strict schemas plus root/`elyra-admin` authorization |
...
- The production daemon socket requires the `elyra` group, which is created and verified in the installation stage.
- Membership in `elyra-admin` is a coarse Unix-group authorization mechanism. Polkit or a similarly interactive authorization system may be evaluated later, but is not silently claimed here.
...
- Actual root-owned `/run/elyra/elyra.sock` startup and systemd confinement require Pardus root verification.
