# ELYRA Stage 1 Repository Audit

Audit basis: `Elyra-main3(2).zip` and the supplied ELYRA Çalışma Planı.
...
| Daemon imports packages that do not exist | `src/daemon/daemon.py:10-19` | Critical | Daemon cannot be imported or started | Created `src/elyra/` package tree and corrected imports |
...
| systemd `ExecStart` targets a nonexistent source path | `src/daemon/daemon.service:9` | High | Service cannot start | Pointed service to `python -m elyra.service.daemon` with the installed `src` path |
...
|---|---|---|
| `src/daemon/analyzer.py` | `src/elyra/analyzer/pre_execution.py` | Preserved; canonicalisation deferred |
| `src/daemon/scanner.py` | `src/elyra/analyzer/static_analyzer.py` | Preserved; entropy/static/scoring split deferred |
| `src/daemon/scoring.py` | `src/elyra/scoring/runtime_engine.py` | Preserved; redesign deferred |
| `src/daemon/controller.py` | `src/elyra/monitor/fanotify/controller.py` | Imports/name fixed |
| `src/daemon/policy.py` | `src/elyra/monitor/fanotify/policy.py` | Moved; policy review deferred |
| `src/daemon/fanotify_syscalls.py` | `src/elyra/monitor/fanotify/syscalls.py` | Moved; kernel verification deferred |
| `src/daemon/loader.py` | `src/elyra/monitor/ebpf/loader.py` | Syntax/import defect repaired |
| `src/daemon/collector.py` | `src/elyra/monitor/ebpf/collector.py` | Optional BCC import; behaviour deferred |
| `src/daemon/probes.c` | `src/elyra/monitor/ebpf/probes.c` | Naming fixed; probe review deferred |
| `src/daemon/quarantine_manager.py` | `src/elyra/response/quarantine_manager.py` | Moved; security review deferred |
| `src/daemon/daemon.py` | `src/elyra/service/daemon.py` | Entry/import ownership repaired |
| `src/daemon/ipc_server.py` | `src/elyra/service/ipc_server.py` | Moved; schema/security review deferred |
| `src/daemon/authorization.py` | `src/elyra/service/authorization.py` | Moved; group model review deferred |
| `src/daemon/audit_logger.py` | `src/elyra/audit/audit_logger.py` | Moved; integrity/rotation review deferred |
| `src/gui/**` | `src/elyra/gui/**` | Package imports/naming repaired; integration deferred |
| `src/daemon/daemon.service` | `packaging/systemd/elyra.service` | Path/name repaired; not run on Pardus |
| `kurulum.sh` | `scripts/install.sh` | Path/name repaired; not run on Pardus |
| `test_scanner.py` | removed | Not an assertion-based test; outside agreed ELYRA workflow |
...
| Import every `elyra.*` Python module | Passed | Verified by automated test on generic Linux |
| Active source filenames contain no `Elyra` naming | Passed | Verified by automated test |
