...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="elyra-stage8-controller-") as temp:
        root = Path(temp)
        ...
        record = records.get_nowait()
        ok = analyzer.calls == 0 and record["reason"] == "ELYRA_PROCESS_EXCLUDED"
        results.append({"scenario": "elyra_self_exclusion", "status": "OK" if ok else "FAIL"})
