...
def fanotify(pid: int, score: int) -> dict:
    return {
        "pid": pid,
        "path": f"/tmp/elyra-stage10-safe-{pid}",
        "score": score,
...
def run_case(engine: ExecutionCorrelationEngine, pid: int, pre_score: int, events: list[dict]) -> dict:
    ...
    engine.ingest_runtime_event(
        event(
            "PROCESS_EXEC",
            pid,
            executable_identifier=f"/tmp/elyra-stage10-safe-{pid}",
        )
    )
...
def main() -> None:
    ...
    parent = engine.ingest_runtime_event(
        event("PROCESS_EXEC", 4200, executable_identifier="/tmp/elyra-stage10-safe-4200")
    )["state"]
