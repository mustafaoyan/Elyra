...
INSTALL_ROOT=/opt/elyra
RELEASES_DIR="$INSTALL_ROOT/releases"
CURRENT_LINK="$INSTALL_ROOT/current"
SERVICE_FILE=/etc/systemd/system/elyra.service
GUI_LAUNCHER=/usr/local/bin/elyra-gui
LEGACY_GUI_LAUNCHER=/usr/local/bin/elyra-gui
DESKTOP_ENTRY=/usr/share/applications/elyra.desktop
SYSUSERS_FILE=/usr/lib/sysusers.d/elyra.conf
...
Options:
  --user USER              Add USER to the unprivileged 'elyra' IPC group.
  --admin-user USER        Explicitly add USER to the sensitive 'elyra-admin' group.
...
Safe defaults:
  ...
  * no user is added to elyra-admin unless --admin-user is supplied.
EOF
}
...
if [[ ${EUID} -ne 0 ]]; then
    echo "Elyra installation requires root: sudo ./scripts/install.sh" >&2
    exit 1
fi
...
create_groups() {
    ...
    install -m 0644 "$SOURCE_DIR/packaging/sysusers/elyra.conf" "$SYSUSERS_FILE"
    if command -v systemd-sysusers >/dev/null 2>&1; then
        systemd-sysusers "$SYSUSERS_FILE"
    else
        getent group elyra >/dev/null || groupadd --system elyra
        getent group elyra-admin >/dev/null || groupadd --system elyra-admin
    fi
    getent group elyra >/dev/null || { echo "Failed to create elyra group" >&2; exit 1; }
    getent group elyra-admin >/dev/null || { echo "Failed to create elyra-admin group" >&2; exit 1; }
}
...
rollback() {
    ...
    echo "Elyra installation failed; attempting rollback." >&2
    ...
    if command -v systemctl >/dev/null 2>&1; then
        ...
        if [[ -n "$PREVIOUS_TARGET" && $START_SERVICE -eq 1 ]]; then
            systemctl restart elyra.service >/dev/null 2>&1 || true
        fi
    fi
    exit "$exit_code"
}
...
if [[ -n "$GUI_USER" ]]; then
    usermod -a -G elyra "$GUI_USER"
fi
if [[ -n "$ADMIN_USER" ]]; then
    usermod -a -G elyra-admin "$ADMIN_USER"
fi
...
if command -v systemctl >/dev/null 2>&1 && systemctl is-active --quiet elyra.service; then
    systemctl stop elyra.service
fi
...
from elyra.service.install_layout import INSTALLATION_MANIFEST, installation_manifest
...
for entry in elyra-daemon elyra-gui elyra-preflight elyra-install-check elyra-daemon elyra-gui; do
...
if ! find "$NEW_RELEASE/.venv/lib" -type f \
    -path '*/site-packages/elyra/monitor/ebpf/probes.c' -print -quit | grep -q .; then
    # A source-tree fallback remains available for editable/build-backend differences.
    [[ -f "$NEW_RELEASE/src/elyra/monitor/ebpf/probes.c" ]] || {
...
install -m 0644 "$NEW_RELEASE/packaging/systemd/elyra.service" "$SERVICE_FILE"
install -m 0755 "$NEW_RELEASE/scripts/launch_gui.sh" "$GUI_LAUNCHER"
install -m 0644 "$NEW_RELEASE/packaging/desktop/elyra.desktop" "$DESKTOP_ENTRY"
ln -sfn "$GUI_LAUNCHER" "$LEGACY_GUI_LAUNCHER"

systemctl daemon-reload
if [[ $ENABLE_SERVICE -eq 1 ]]; then
    systemctl enable elyra.service
fi
if [[ $START_SERVICE -eq 1 ]]; then
    systemctl restart elyra.service
    if ! systemctl is-active --quiet elyra.service; then
        systemctl status elyra.service --no-pager >&2 || true
        journalctl -u elyra.service -n 80 --no-pager >&2 || true
        false
    fi
fi
...
echo "Elyra installed release: $RELEASE_NAME"
echo "Current release: $(readlink -f "$CURRENT_LINK")"
echo "Service status: sudo systemctl status elyra.service"
echo "Service logs: sudo journalctl -u elyra.service"
echo "GUI launcher: elyra-gui (legacy alias: elyra-gui)"
if [[ -n "$GUI_USER" ]]; then
    echo "User '$GUI_USER' was added to group 'elyra'. Log out and back in before launching the GUI."
fi
if [[ -n "$ADMIN_USER" ]]; then
    echo "WARNING: '$ADMIN_USER' was explicitly added to the sensitive 'elyra-admin' group."
fi
