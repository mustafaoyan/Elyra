...
from elyra.service.integration_check import (
...
def test_journal_error_detector_accepts_normal_monitor_logs() -> None:
    failed, markers = journal_has_forbidden_error(
        "INFO elyra.monitor.fanotify.controller: fanotify ALLOW: path=/tmp/safe"
    )
...
def test_stage14_checker_preserves_safe_production_defaults() -> None:
    source = (ROOT / "src/elyra/service/integration_check.py").read_text(encoding="utf-8")
...
def test_stage14_pyproject_exposes_installed_integration_check() -> None:
    ...
    assert 'elyra-integration-check = "elyra.service.integration_check:main"' in pyproject
