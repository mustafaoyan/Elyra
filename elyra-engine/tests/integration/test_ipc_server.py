...
from elyra.service.ipc_client import IpcClient, IpcRemoteError
from elyra.service.ipc_protocol import MAX_REQUEST_BYTES, PROTOCOL_VERSION, build_request
from elyra.service.ipc_server import IpcConfigurationError, IpcServer
...
def running_server(tmp_path: Path):
    socket_path = tmp_path / "run" / "elyra.sock"
...
def test_remote_authorization_error_is_structured(tmp_path: Path) -> None:
    socket_path = tmp_path / "elyra.sock"

    def deny(_requester, _action):
        from elyra.service.authorization import AuthorizationError
