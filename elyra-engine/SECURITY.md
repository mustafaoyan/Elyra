...
- Sensitive actions require root or explicit `elyra-admin` membership.
- `elyra-admin` must never be assigned automatically.
...
1. Stop the service: `sudo systemctl stop elyra.service`.
